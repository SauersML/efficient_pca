...
#[cfg(test)]
mod pca_bench_tests {
    // To benchmark different linear algebra backends (e.g., ndarray-linalg vs. faer):
    //
    // 1. Compile and test with the faer backend enabled:
    //    cargo test --release --features backend_faer -- --nocapture bench_all_scenarios_in_one
    //    (Ensure a clean build if switching from a default build: `cargo clean` might be needed first)
    //    Results will be printed to the console and appended to "benchmark_results.csv".
    //
    // 2. Clean the build:
    //    cargo clean
    //
    // 3. Compile and test with the default ndarray-linalg backend (faer feature disabled):
    //    cargo test --release -- --nocapture bench_all_scenarios_in_one
    //    Results will be printed to the console and appended to "benchmark_results.csv".
    //
    // The "benchmark_results.csv" file will then contain results from both backends,
    // identifiable by the "Backend" column. Analyze this CSV file to compare performance.
    use super::*;
    use ndarray::Array2;
    use rand::distributions::Uniform;
    use rand::{Rng, SeedableRng};
    use rand_chacha::ChaCha8Rng;
    use std::fs::OpenOptions; // For CSV writing
    use std::io::Write; // For CSV writing
    use std::path::Path; // For CSV writing
    use std::time::Instant;
    use sysinfo::System;
    ...
    struct BenchResult {
        ...
        rfit_memory_kb: u64,
        backend_name: String,
        num_components_override: Option<usize>, // New field
    }
...
/// Generates random data of shape (n_samples x n_features) with values 0, 1, or 2 (as f64), seeded for reproducibility.
    ...
    fn benchmark_pca(
        use_rfit: bool,
        data: &Array2<f64>,
        n_components_override: Option<usize>, // Changed parameter
        n_oversamples_for_rfit: usize,      // Clarified name
        seed_for_rfit: u64,                 // Clarified name
    ) -> (f64, u64) {
        ...
        if use_rfit {
            ...
            // For 'fit', n_components_override is not directly used to limit fit's components,
            // as fit(tolerance=None) computes all possible components.
            // The override might be relevant for assertions or if fit behavior changes.
    ...
    fn run_scenario(
        ...
        n_components_override: Option<usize>, // New parameter
    ) -> BenchResult {
        println!(
            "\n=== Test Case: {} ({} samples x {} features, Components Override: {:?}) ===", // Updated log
            scenario_name, n_samples, n_features, n_components_override
        );

        // Generate data based on the specified source type
        let data = match data_source_type {
            DataSource::Dense012 => generate_random_data(n_samples, n_features, seed),
            DataSource::Sparse012(s) => {
                generate_sparse_random_data(n_samples, n_features, s, seed)
            }
        ...
        let oversamples_for_rfit = 10; // Default value

        // FIT
        let (fit_time, fit_mem_kb) =
            benchmark_pca(false, &data, n_components_override, oversamples_for_rfit, seed);
        println!(
        ...
        // RFIT
        let (rfit_time, rfit_mem_kb) =
            benchmark_pca(true, &data, n_components_override, oversamples_for_rfit, seed);
        println!(
        ...
        let backend_name = if cfg!(feature = "backend_faer") {
            "faer".to_string()
        } else {
            "ndarray".to_string() // Assuming ndarray is the default
        };
        ...
        BenchResult {
            ...
            backend_name,
            num_components_override, // Populate new field
        }
    }
    ...
    fn print_summary_table(results: &[BenchResult]) {
        ...
        println!("=========================================================================================="); // Adjusted for new column
    }
    ...
    fn bench_all_scenarios_in_one() {
        ...
        let scenarios = vec![
            ...
            ("Wide-XL", 100, 100000, 1234, DataSource::Dense012, None),
        ...
        // Run them
        for (name, samps, feats, seed, data_source, n_comp_override) in scenarios {
            let bench_result = run_scenario(name, samps, feats, seed, data_source, n_comp_override);
            results.push(bench_result);
        }

        // Print final summary
        print_summary_table(&results);
        if let Err(e) = append_results_to_csv(&results, "benchmark_results.csv") {
    ...
    #[test]
    fn test_generate_sparse_data_values() {
        let n_samples = 100;
        let n_features = 100;
        let sparsity_target = 0.9;
        let seed = 42;
        let data = generate_sparse_random_data(n_samples, n_features, sparsity_target, seed);

        let zero_count = data.iter().filter(|&&x| x == 0.0).count();
        let total_elements = data.len();
        let measured_sparsity = zero_count as f64 / total_elements as f64;

        // Check if measured sparsity is within a tolerance (e.g., +/- 5%)
        assert!(
            (measured_sparsity - sparsity_target).abs() < 0.05,
            "Measured sparsity {} not close to target {} (tolerance 0.05)",
            measured_sparsity,
            sparsity_target
        );

        let mut non_zero_values_are_correct = true;
        for &val in data.iter() {
            if val != 0.0 {
                if val != 1.0 && val != 2.0 {
                    non_zero_values_are_correct = false;
                    eprintln!("Non-zero value {} is not 1.0 or 2.0", val);
                    break;
                }
            }
        }
        assert!(
            non_zero_values_are_correct,
            "Found non-zero values that are not 1.0 or 2.0"
        );
    }

    #[test]
    fn test_generate_low_variance_data_values() {
        let n_samples = 100;
        let n_features = 200;
        let fraction_low_var_target = 0.5;
        let majority_freq_target = 0.9; // In low-var features, 0.0 should appear this often
        let seed = 43;

        let data = generate_low_variance_data(
            n_samples,
            n_features,
            fraction_low_var_target,
            majority_freq_target,
            seed,
        );

        let mut actual_low_var_feature_count = 0;
        let mut features_identified_as_low_var_indices = Vec::new();

        for j in 0..n_features {
            let feature_col = data.column(j);
            let mut zero_count = 0;
            for &val in feature_col.iter() {
                if val == 0.0 {
                    zero_count += 1;
                } else {
                    // For any feature, minority values must be 1.0 or 2.0
                    assert!(val == 1.0 || val == 2.0, "Minority value {} in feature {} is not 1.0 or 2.0", val, j);
                }
            }
            let zero_proportion = zero_count as f64 / n_samples as f64;

            // Heuristic to identify if a feature behaves like a low-variance one
            // A low-variance feature should have a high proportion of the majority value (0.0)
            // A normal-variance feature should have a more even distribution.
            // This threshold (e.g., 0.80) helps distinguish them for testing purposes.
            // It's greater than 1/3 (random chance for 0,1,2) and less than majority_freq_target.
            if zero_proportion > (1.0/3.0 + (majority_freq_target - 1.0/3.0)/2.0) { // Heuristic threshold
                actual_low_var_feature_count += 1;
                features_identified_as_low_var_indices.push(j);
            }
        }

        let actual_fraction_low_var = actual_low_var_feature_count as f64 / n_features as f64;
        // Check if the proportion of features that *behave* as low-variance is statistically close to the target.
        // Using a wider tolerance here (e.g., 0.20) because the feature type itself is probabilistic.
        assert!(
            (actual_fraction_low_var - fraction_low_var_target).abs() < 0.20,
            "Actual fraction of detected low-var features ({}) not close to target ({}). Tolerance 0.20. Detected low-var indices: {:?}",
            actual_fraction_low_var,
            fraction_low_var_target,
            features_identified_as_low_var_indices
        );

        // For features that were identified as low-variance by our heuristic,
        // check the frequency of the majority value (0.0).
        for &j in &features_identified_as_low_var_indices {
            let feature_col = data.column(j);
            let zero_count = feature_col.iter().filter(|&&v| v == 0.0).count();
            let zero_proportion_in_low_var_feature = zero_count as f64 / n_samples as f64;
            assert!(
                (zero_proportion_in_low_var_feature - majority_freq_target).abs() < 0.1, // Tighter tolerance for value distribution within a known low-var type
                "In detected low-var feature {}, zero proportion {} not close to target {}. Tolerance 0.1",
                j, zero_proportion_in_low_var_feature, majority_freq_target
            );
        }
    }
}
